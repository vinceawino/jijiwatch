...
function getIncidents() {
    ...
    request = $.ajax({
        url: "http://localhost/jijiwatch/index.php/api/getIncidents",
        type: "get",
    });
...
function getIncidentTypes() {
    ...
    // Fire off the request
    request = $.ajax({
        url: "http://localhost/jijiwatch/index.php/api/getIncidentTypes",
        type: "get",
    });

    request.done(function (response, textStatus, jqXHR){
        incidentTypes=JSON.parse(response).types;
        getIncidents();
    });
...
function simpleChart() {
    ...
    for (var i=0;i<incidentTypes.length;i++){
        var subSum=new Object();
        subSum.incidentType=incidentTypes[i].description;
        subSum.number=0;
        summ.push(subSum);
        var leng={};
        leng.data=summ;
        leng.label=incidentTypes[i].description;
        leng.color=getRandomColor();
        legend.push(leng);
    }
    for (var i=0;i<incidents.length;i++){
        var inc=incidents[i];
        for (var j=0;j<summ.length;j++){
            if(summ[j].incidentType==inc.incident_type){
                ++(summ[j].number);
                subSum.day=getNumberOfDays(incidents[0].recordtime,inc.recordtime);
               }
        }
    ...
    for(var i=0;i<summ.length;i++){
        flash.push([summ[i].day,summ[i].number]);
        html5.push([summ[i].day,summ[i].number]);
    }
    console.log(flash);
