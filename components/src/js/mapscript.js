	...
	function(
		...
		on(dom.byId("clearBtn"), "click", function(){
			if (routeGraphicLayer.graphics.length != 0 && resultsGraphic.length == 0) {
				...
				routeGraphicLayer.clear();
				dom.byId("directionsDiv").innerHTML = "";
			}else if(generatedPolygons.length != 0 && resultsGraphic.length == 0){
				map.graphics.clear();
				generatedPolygons.length = 0;
			}else{
		...
		function performServiceAreaTask (){
			...
			serviceAreaTask.solve(params1, function(solveResult){
				...
				var polygonSymbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,new SimpleLineSymbol(SimpleFillSymbol.STYLE_SOLID, new Color([232,104,80]), 2), new Color([232,104,80,0.25]));
		...
		function socketConnect() {
			...
			channel.bind('my_event', function(data) {
				...
				var inPoint = {
					...
					"infoTemplate": {
						"title" : "Incident",
						"content" : "ID: ${id}<br/>${incident}<br/>Date/Time: ${date_time}<br/>Phone No: ${entered_msisdn}<br/><br/><img src='" + "http://69.64.65.196:6060/demo/" + "${url}' alt='Picture was not taken' style='width:170px;height:175;'><br/>"
					}
				};
				...
				table.bootstrapTable('insertRow', {index: 1, row: data[0]});
			});
		...
		on($("#mapSelected"), "click", function(){
			...
			array.forEach(selectedRow, function(obj){
				...
				var inPoint = {
					...
					"infoTemplate": {
						"title" : "Incident",
						"content" : "ID: ${id}<br/>${incident}<br/>Date/Time: ${date_time}<br/>Phone No: ${entered_msisdn}<br/><br/><img src='" + "http://69.64.65.196:6060/demo/" + "${url}' alt='Picture was not taken' style='width:170px;height:175;'><br/><br/>"
					}
				};
		...
		/*Table
		 Export
		 */
		$('#myToolbar').find('select').change(function () {
		...
		//Dispatch
		var incidentID;
		on($("#dispatchBtn"), "click", function(){
			if(resultsGraphic.length == 1){
				array.forEach(resultsGraphic, function(graphicObj){
					incidentID = graphicObj.attributes.id;
					window.location.replace("http://localhost/jijiwatch/index.php/en/incidents/"+incidentID);
					//console.log(incidentID);
				});
			}else{
				$("#dispatchWarningDialog").modal();
			}
		});
	});